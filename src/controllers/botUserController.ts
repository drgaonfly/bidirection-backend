...
export const generateBoundProxy = handleAsync(
  async (req: RequestCustom, res: Response) => {
    ...
    if (!pendingApplication) {
      res.status(400);
      throw new Error('没有找到待处理的申请或申请已被处理');
    }

    console.log('开始处理申请:', pendingApplication.id);
    ...
    try {
      ...
      if (existingBotUserWithProxy) {
        // 如果用户已有代理，将申请状态改回 pending
        await Application.findByIdAndUpdate(pendingApplication._id, {
          status: 'pending',
        });
        res.status(400);
      ...
      // 更新申请状态为已批准
      console.log('更新申请状态为已批准:', pendingApplication.id);
      ...
      // 如果处理失败，将申请状态重置为 pending
      console.error('代理生成失败，重置申请状态:', error);
      await Application.findByIdAndUpdate(pendingApplication._id, {
        status: 'pending',
      });
      throw error;
