...
export async function checkWallets() {
  try {
    console.log('[checkTrxWallets] 开始检查转账...');
    ...
    for (const wallet of wallets) {
      ...
      let transfers = [];
      try {
        const trxTransfers = await getTrxTransfers(address);
        const usdtTransfers = await getUSDTTransfers(address);

        // 合并
        transfers = [...trxTransfers, ...usdtTransfers];
      } catch (err) {
      ...
      for (const transfer of transfers) {
        ...
        try {
          ...
          console.warn(`[checkWallets] 获取余额失败:`, err);
        }
        ...
        try {
          ...
          console.error(`[checkWallets] 通知用户 ${botUser.id} 失败:`, err);
        }
      }
    }

    console.log('[checkWallets] TRX 转账检查完成');
  } catch (error) {
    console.error('[checkWallets] 执行异常:', error);
  }
}
